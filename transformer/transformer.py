...
class Transformer(nn.Cell):
    ...
    def __init__(self, encoder=None, decoder=None):
        ...
        if encoder is not None and decoder is not None:
            ...
            #     if p.dim() > 1:
            #         p = initializer(XavierUniform(), p.shape, p.dtype)
        else:
            self.encoder = Encoder()
            self.decoder = Decoder()

    def construct(self, padded_input, input_lengths, padded_target):
        '''
        训练过程
    ...
    def recognize(self, input, input_length, char_list, args):
        ...
        encoder_outputs, *_ = self.encoder(ops.expand_dims(input, 0), input_length)
